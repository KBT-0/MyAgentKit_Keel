...
def main():
    ...
    timeout = int(os.environ.get("REVIEW_TIMEOUT_SECONDS", "600"))
    ...
    with tempfile.TemporaryDirectory(prefix="myagentkit-codex-") as tmp:
        ...
        if not reason and (len(verdicts) != 1 or not any(v.get("type") == "turn.completed" for v in values)):
    ...
    archived = None
    try:
        agent_usage.write_evidence(repo, report, report_text)
        archived = str(report)
    ...
    print("review invocation: " + json.dumps(
        {"status": status, "failure_kind": reason, "evidence": archived,
